...
def repo_docs_dir() -> Path | None:
    """Locate `backend-java/docs` by walking parents from this file, or `WATERMARK_DOCS_PATH`."""
...
def load_chunks(max_chunk_chars: int = 1400, overlap: int = 200) -> list[DocChunk]:
    ...
    if base is None:
        log.warning(
            "backend-java/docs not found (set WATERMARK_DOCS_PATH or build from repo root). RAG disabled."
        )
        return chunks
